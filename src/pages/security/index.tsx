...
function Main() {
  ...
  return (
    <>
      <h2 className="mt-10 text-lg font-medium intro-y">Security Feature</h2>
              ...
              {users.map((user: any, key) => (
                <Table.Tr key={key} className="intro-x">
                  ...
                  <Table.Td className="first:rounded-l-md last:rounded-r-md bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b]">
                    <div
                      className={user.isPasswordChanged ? "flex items-center text-success" : "flex items-center text-danger"}
                    >
                      <Lucide icon={user.isPasswordChanged ? "CheckSquare" : "XSquare"} className="w-4 h-4 mr-2" />
                      {user.phoneNumber}
                    </div>
                  </Table.Td>
